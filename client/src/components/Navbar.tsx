...
const Navbar = () => {
  ...
  return (
    <header
      className={`bg-white shadow-md fixed w-full top-0 z-50 transition-shadow ${isScrolled ? "shadow-lg" : "shadow-md"}`}
    >
      <nav className="container mx-auto px-4 py-3 flex justify-between items-center">
        <Link href="/" className="flex items-center space-x-2">
          ...
          />
          <span className="text-dark font-heading font-semibold text-lg hidden sm:block"></span>
        </Link>
      ...
      {/* Mobile Navigation */}
      <div
        className={`md:hidden bg-white shadow-lg absolute w-full left-0 top-full transition-all duration-300 ${mobileMenuOpen ? "max-h-[500px] opacity-100" : "max-h-0 opacity-0 overflow-hidden"}`}
      >
